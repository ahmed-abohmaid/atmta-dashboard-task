...
export function IconPicker({
  ...
  const filteredIcons = useMemo(() => {
    ...
    const unique = [
      currentIconName,
      ...matches.filter((name) => name !== currentIconName),
    ];
          ...
          <PopoverTrigger
            render={
              <Button
                ...
                className="size-9 p-0 rounded-lg bg-secondary/60 text-primary border-border/80 hover:bg-secondary/90 transition-colors shrink-0"
              >
          ...
          <PopoverContent align="start" className="w-72 p-3">
            <div className="flex flex-col gap-2.5">
              <div className="relative">
                <SearchIcon className="absolute inset-y-0 inset-s-0 my-auto ms-2 size-3 text-muted-foreground pointer-events-none" />
                <Input
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder={placeholder}
                  className="h-7 ps-7 text-xs placeholder:text-xs placeholder:text-muted-foreground/50"
                  autoFocus
                />
              </div>
