...
- [ ] **Hard rules (must hold, testable):**
  - [x] Permissions enforced across the _whole_ UI: buttons/links hidden or disabled without access; direct URL access to a forbidden page shows a clear forbidden state (not a silent redirect or crash).
...
- [ ] CSV export of the _filtered_ list — gated behind the `Export Vendors` permission specifically (independent of general vendor CRUD access).
